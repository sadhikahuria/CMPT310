...
class VacuumPlanning(Problem):
    ...
    def findMinManhattanDist(self, pos):
        ...
        for room in set(self.env.dirtyRooms):
    ...
    def findMinEuclidDist(self, pos):
        ...
        """Use distance_squared() for Euclid distance"""
        mindist = float('inf')
    ...
    while len(frontier):
        ...
        for child in node.expand(problem):
            if tuple(child.state) not in explored and child not in frontier:
                ...
                if f(child) < frontier[child] and tuple(child.state) not in explored:
                    del frontier[child]
                    frontier.append(child)
