...
class RegressionModel(object):
    ...
    def train(self, dataset):
        ...
        while iterations < self.max_iterations:
            ...
            print("Iteration {}: Loss = {:.6f}".format(iterations, avg_loss))
            if avg_loss < 0.02:
                print("Training complete with average loss {:.6f}".format(avg_loss))
                break
            iterations += 1
    ...
    def train(self, dataset):
        ...
        while iterations < self.max_iterations:
            for x, y in dataset.iterate_once(self.batch_size):
                ...
                self.W1.update(grads[0], -0.01)
                self.b1.update(grads[1], -0.01)
                self.W2.update(grads[2], -0.01)
                self.b2.update(grads[3], -0.01)
                self.W3.update(grads[4], -0.01)
                self.b3.update(grads[5], -0.01)
...
class LanguageIDModel(object):
    ...
    def train(self, dataset):
        ...
        for epoch in range(self.max_epochs):
            ...
            value_acc = dataset.get_validation_accuracy()
            if value_acc >- self.target_accuracy:
                break
