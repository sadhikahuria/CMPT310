...
def minmax_cutoff(game, state):
    ...
    def max_value(state,d):
        ...
        # If we reach terminal state or reach the deep limit then get this block

        if (game.terminal_test(state) or d == 0):
            
            # If we reach terminal state then retrun the utility
            if game.terminal_test(state):
                return game.utility(state,player)
            # If reach the deep limit then return eval function's result

            else:

                return game.eval1(state)

        v = -np.inf # taking the value as negative infinity

        for act in game.actions(state):

            v = max(v, min_value(game.result(state, act), d -1))

        return v

    def min_value(state, d):

        # If we reach terminal state or reach the deep limit then enter this block
        if (game.terminal_test(state) or d == 0):

            # If we reach terminal state then return the utility
            if game.terminal_test(state):
                return game.utility(state, player)
        ...
        v = np.inf # taking the value as positive infinity

        for act in game.actions(state):
            v = min(v, max_value(game.result(state, act), d -1))
        return v

    # Body of minmax_cutoff:
    best_action = None
    alpha = -np.inf # Taking the alpha value

    # iterating each elements from actions
    for act in game.actions(state):
        act_value = min_value(game.result(state, act), game.d)
...
def alpha_beta(game, state):
    ...
    beta = np.inf
    best_action = None
     # helper functions for alpha_beta
    def max_value(state,alpha,beta):
        ...
        # iterating each elements from actions
        for act in game.actions(state):
            v = max(v, min_value(game.result(state,act),alpha,beta))
            if v >= beta:
                return v
            alpha = max(alpha,v)
        return v
    ...
    def min_value(state,alpha,beta):
        ...
        # iterating each elements from actions
        for act in game.actions(state):
            v = min(v, max_value(game.result(state,act),alpha, beta))
            if v <= alpha:
                return v
            beta = min(beta , v)
        return v
    ...
    for act in game.actions(state):
        ...
        v = min_value(game.result(state, act), alpha,beta)
...
def alpha_beta_cutoff(game, state):
    ...
    beta = np.inf
    best_action = None
# helper functions for alpha_beta_cutoff
    def max_value(state,alpha,beta,depth):
        ...
        # iterating each elements from actions
        for act in game.actions(state):
            v = max(v,min_value(game.result(state,act),alpha, beta, depth -1))
            if v >= beta:
                return v
            alpha = max(alpha, v)
        return v
    ...
    def min_value(state, alpha, beta, depth):
        ...
        # iterating each elements from actions
        for act in game.actions(state):
            v = min(v, max_value(game.result(state,act), alpha, beta, depth - 1))
            if v <= alpha:
                return v
            beta = min(beta, v)
        return v
    print("Your code goes here 15pt.")
     ...
     # iterating each elements from actions
    for act in game.actions(state):
        v = min_value(game.result(state, act), alpha, beta, game.d )
        # Finding the best action
        if v > alpha:
            alpha = v
            best_action = act
...
def alpha_beta_player(game, state):
    ...
    def iterative_deepening(game, state, end_time):
        ...
        return best_move
    return iterative_deepening(game, state, end)
...
def minmax_player(game, state):
    ...
    move = None
    print("Your code goes here 5pt.")
# iterating till time comes to an end
    depth = 0
    while time.perf_counter() < end:
        ...
        if move is None:
            break
    # If move is still not found then use a random move
    if move is None:
...
class TicTacToe(Game):
    ...
    def eval1(self, state):
        ...
        score = 0
        # For each move in the board calculating score
        for m in state.board:
            # Adding score if it is X
            if state.board[m] == 'X':
                score = score + possiblekComplete(m,state.board, 'X', self.k-1)
            # Deducting score if it is o
            elif state.board[m] == 'O':
                score = score - possiblekComplete(m, state.board, 'O', self.k - 1)
